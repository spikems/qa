...
#
normdict = {}
industry_index = {2:'car.dict',7:'makeup.dict',0:'newword.dict'}

def load_industrydict(files):
    for file in files:
        if type(file) == str:
            file = int(file)
        infile = os.path.join(cur_path,'industrydict/%s'%industry_index[file])
        jieba.load_userdict(infile)
...
def suggest_freq(self, segment, tune=False):
    ...
    if isinstance(segment, string_types):
        word = segment
        for seg in self.norm_cut(word, HMM=False):
