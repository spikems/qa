...
#            self.vectorizer = TfidfVectorizer(analyzer=str.split, sublinear_tf=True, max_df=max_df, ngram_range=ngram_range, vocabulary = self.vocabulary)
            ...
            self.vectorizer = CountVectorizer(binary = True, max_df=0.45, ngram_range=(1,2), vocabulary = self.vocabulary, token_pattern=r'[^ ]+', min_df=5)
#            self.vectorizer = CountVectorizer(analyzer=str.split,binary = True, max_df=max_df, ngram_range=ngram_range, vocabulary = self.vocabulary)
    ...
    def train_model(self, clf, savename):
        ...
        if hasattr(clf, 'coef_'):
            with open(savename + '_feature.coef', 'w') as cof:
                for i, category in enumerate(categories):
                    ...
                    for _id in range(clf.coef_[i].shape[0]):
                        ...
                        if type(feature_names[vid]) in [np.unicode_, unicode]:
                            cof.write("%s\t%s\t%s\n" % (
                            category, feature_names[vid].encode('utf-8') if feature_names is not None else vid,
                            clf.coef_[i][vid]))
                        else:
                            cof.write("%s\t%s\t%s\n" % (
    ...
    def predict(self, clf = None, savename = 'demo-predict'):
        ...
        with open(name + '.cm', 'w') as cmf:
            ...
            for item in cmstr:
                cmat.append('row')
                cmat.append("\t".join([str(ele) for ele in item]))

            res = "".join(cmat)
            cmf.write('confusion_matrix:%s\n' % (res))
            cmf.write('%s\n'%(cmstr))
            cmf.write("precision:%s\n" % self.format_effect(precision))
            cmf.write("recall:%s\n" % self.format_effect(recall))
            cmf.write("f1:%s\n" % self.format_effect(f1_score))
            cmf.write("accuracy:%0.3f\n" % score)
