...
def options():
    ...
    d_info['fs_type'] = 'no'
    d_info['use_word2vec'] = ""
    d_info['vectorizer_type'] = 'tfidf'
    d_info['ngram_range'] = "1,1"
    d_info['classifier'] = "lr"
    d_info['vocabulary'] = ""
    d_info['testmodel'] = ""
    d_info["trainfile"] = ""
    d_info["testfile"] = ""
    ...
    d_info['bin_class'] = -999
    d_info['n_features'] = 0

    d_info["recordid"] = False
    d_info["balanced"] = False
    d_info["debug"] = False

    return d_info


def train(algorithm = '', trainfile = '', model_path = ''):
    # get the classifier
    # specification for the full name
    # appname_dataset_classifier_balanced_fstype_vectorizertype_nfeatures_ngram

    #1. get configure info
    d_info = options()
    for param in algorithm_conf[algorithm]:
        val = algorithm_conf[algorithm][param]
        d_info[param] = val

    #model name
    fullname, clf = get_classifier(d_info['classifier'], d_info['balanced'])
    fullname = model_path

    # setup the learner
    trainer = Learner(train = True)
    trainer.load_dataset(trainfile, d_info['recordid'], d_info['bin_class'])
    trainer.init_vocabulary(d_info['vocabulary'])
    trainer.init_vectorizer(type = d_info['vectorizer_type'], ngram_range = d_info['ngram_range'])
    trainer.init_featureselector(type = d_info['fs_type'], selectCnt = d_info['n_features'])
    trainer.transform()
    trainer.feature_select(savename = fullname)
    trainer.train_model(clf, fullname)


def t_predict(model_path, temp_path, testfile, algorithm):
    d_info = options()
    for param in algorithm_conf[algorithm]:
        val = algorithm_conf[algorithm][param]
        d_info[param] = val
    ...
    predictor.load_dataset(testfile, d_info['recordid'], d_info['bin_class'])
    predictor.transform()
    predictor.feature_select(savename=temp_path)
    predictor.predict(savename=temp_path)
